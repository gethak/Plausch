...
import { Bookmark, ChevronDown, Heart, Lock, Plus, Scale, ShieldCheck } from 'lucide-react'
...
const annotationIcons = [
  <Bookmark className="h-4 w-4 text-[#e07b39]" />,
  <Scale className="h-4 w-4 text-[#e07b39]" />,
]
...
export default function MobileValues() {
  ...
  const panels = [
    // 1 — no more hallucinations / best answer
    <p className="text-[13px] leading-relaxed text-[#17130e]/70">{t.values.bestAnswerBody}</p>,

    // 2 — every model in one app: grid + picker phone
    ...
    // 4 — annotate / note / remember
    <div>
      <p className="text-[13px] leading-relaxed text-[#17130e]/70">{t.features.rows[0].body}</p>
      <ul className="mt-3 space-y-2.5">
        {t.features.rows[0].bullets.map((label, i) => (
          <li key={label} className="flex items-center gap-2.5">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white shadow-sm ring-1 ring-[#17130e]/5">
              {annotationIcons[i]}
            </span>
            <span className="text-[13px] font-medium text-[#17130e]/75">{label}</span>
          </li>
        ))}
      </ul>
    </div>,

    // 5 — ethical / socialist / pro-humanitarian
      ...
      {t.values.items.map((item, i) => (
        <div key={i} className="overflow-hidden rounded-3xl bg-[#fdf7ef] shadow-sm ring-1 ring-[#17130e]/10">
          ...
          >
            ...
            <span className="flex-1 text-[15px] font-bold leading-snug text-[#17130e]">
              {item.pre}
              {item.accent && <span className="text-[#e07b39]">{item.accent}</span>}
              {item.post}
            </span>
            <ChevronDown
          ...
          {open === i && <div className="px-5 pb-5">{panels[i]}</div>}
        </div>
      ))}
    </div>
  )
